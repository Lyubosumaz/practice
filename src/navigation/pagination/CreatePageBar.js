        ...
        <Fragment>
                ...
                <nav className="pagination-numbers">
                    <div className="pagination-numbers-wrapper">
                        {props.arrayPages.map(page =>
                            <button key={page.id} className={`pagination-numbers-buttons ${page.isActive ? 'active-number-button' : ''}`}>
                                <span>{page.text}</span>
                            </button>
                        )}
                    </div>
            ...
            {props.typeOfBar === 'arrow' &&
                <nav className="pagination-arrow">
                    <button className="pagination-arrow-left action-button">
                    ...
                    <div className="pagination-arrow-wrapper">
                        {props.arrayPages.map((page, index) => {
                            if (props.arrayPages.length - 2 === index) {
                                // adding a element before last element
                                return (
                                    <Fragment>
                                        <button
                                            key={page.id}
                                            className={`pagination-arrow-buttons ${page.isActive ? 'active-arrow-button' : ''}`}
                                        >
                                            <span>{page.text}</span>
                                        </button>
                                        <button
                            ...
                            return <button key={page.id} className={`pagination-arrow-buttons ${page.isActive ? 'active-arrow-button' : ''}`}><span>{page.text}</span></button>
                        }
                        )}
                    </div>
                    <button className="pagination-arrow-right action-button">
