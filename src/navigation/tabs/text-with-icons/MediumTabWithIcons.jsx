...
export default function MediumTabWithIcons() {
    const data = {
        quantity: tabsConfig.MediumTabWithIcons.quantity,
        active: tabsConfig.MediumTabWithIcons.active,
        type: tabsConfig.MediumTabWithIcons.type,
        class: tabsConfig.MediumTabWithIcons.class,
    }
    const validation = data.quantity >= 1 && data.quantity <= 4;
    const number = validation ? data.quantity : tabsConfig.errorMessage;
    const [tabs, setTabs] = useState(SetTabsArray({ numberOfTabs: number, activeIndex: data.active, images: [tabName, secondTab, tabName] }));
